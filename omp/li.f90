...
subroutine zflowdiagnostics

  implicit none
  integer :: m,i,il,ir
  real(8) :: xpdx,wx,kx,p(0:nx)
  complex(8) :: zphi(0:nx-1)
...
!$OMP END PARALLEL DO
  ...
  !set pressure term to either px or .5*(px+py)
  p = px
  !p = 0.5*(px+py)
  ...
  do i=0,nx
    ...
    if (i.eq.0) il=nx-1
    if (i.eq.nx) ir=1
    dp(i)= 0.5*(p(ir)-p(il))/dx
    er(i)= dp(i) - 0.5*(uy(i)*cth - uz(i)*sth)
  end do
